...
def gerer_missing_data(state: SRARState) -> SRARState:
    ...
    reponse = (
        ...
        f"*Le système refuse d'inventer des valeurs (Axiome 3 SRAR-GP).*"
    )
...
def correction_code(state: SRARState) -> SRARState:
    """Renvoie au Coder avec prompt simplifié (sans le code précédent)."""
    print(f"\n  ┌─ [RENÉGOCIATION] Correction du code Python (prompt simplifié)...")
...
3. Applique l'équation
4. Affiche le résultat avec print()
5. PAS de conversion d'unités sauf si l'équation l'exige
6. Format : code Python pur dans un bloc ```python ... ```
...
Reformule un NOUVEAU Blueprint en tenant compte de la critique.
Garde la même structure (5 sections).
...
def router_renegociation(state: SRARState) -> str:
    ...
    derniers_agents = state.get("agents_actives", [])
    if any("failed" in a for a in derniers_agents[-3:]):
        print(f"  │  → Échec technique récent → fin")
        return "end"
